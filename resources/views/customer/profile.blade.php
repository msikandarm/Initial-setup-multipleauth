...
@section('content')
  <x-section-container column="col-xl-6 col-lg-7">
    ...
    <x-form action="{{ route('profile.update') }}" method="put" customrules>
      <x-form-group label="{{ __('Name') }}" inputId="name">
        <x-input name="name" :value="auth()->guard('customer')->user()->name" required />
      </x-form-group>

      <x-form-group label="{{ __('Email') }}" inputId="email">
        <x-input type="email" name="email" :value="auth()->guard('customer')->user()->email" required />
      </x-form-group>
